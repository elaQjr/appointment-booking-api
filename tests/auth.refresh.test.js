...
describe('GET /api/auth/refresh', () => {
    ...
    test('should return 401 when refresh token is missing', async () => {
        ...
        expect(res.body.message).toMatch(/Access Denied. No refresh token provided./i);
    });
    ...
    test('should return 200 and new access token when refresh token is valid', async () => {
        ...
        expect(tokenUtils.generateAccessToken).toHaveBeenCalledWith(user._id.toString());
        expect(tokenUtils.generateRefreshToken).toHaveBeenCalledWith(user._id.toString());
    });
});
